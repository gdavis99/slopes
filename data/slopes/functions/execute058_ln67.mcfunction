...
execute store result score @s blocks7 run scoreboard players get @e[type=minecraft:armor_stand,tag=MiniBlock,limit=1,scores={_age=1}] _id
scoreboard players operation Global _id = @s blocks7
execute as @e[type=minecraft:armor_stand,tag=MiniBlock] if score @s _id = Global _id run function slopes:execute057_ln69