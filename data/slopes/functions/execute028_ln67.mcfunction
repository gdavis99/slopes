...
execute store result score @s blocks1 run scoreboard players get @e[type=minecraft:armor_stand,tag=MiniBlock,limit=1,scores={_age=1}] _id
scoreboard players operation Global _id = @s blocks1
execute as @e[type=minecraft:armor_stand,tag=MiniBlock] if score @s _id = Global _id run function slopes:execute027_ln69